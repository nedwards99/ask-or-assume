...
class IntentAgent(Agent):
    ...
    def __init__(
        ...
        shared_condenser: Condenser | None = None,
    ) -> None:
        ...
        logger.info(
            "[IntentAgent] enable_plan_mode=%s system_prompt=%s",
            self.config.enable_plan_mode,
        ...
        if shared_condenser is not None:
            self.condenser = shared_condenser
            logger.debug(
                'Reusing shared condenser %s for IntentAgent', type(self.condenser)
            )
        else:
    ...
    def reset(self) -> None:
        ...
        self.awaiting_user_response = False
        self.awaiting_user_response_id = None
    ...
    def step(self, state: State) -> 'Action':
        ...
        if self.awaiting_user_response:
            ...
            if latest_id is None or latest_id == self.awaiting_user_response_id:
                return AgentThinkAction(thought='Waiting for user clarification.')

            summary = latest_user_message.content if latest_user_message else ''
            finish_action = AgentFinishAction(
                final_thought='Intent clarifications recorded.',
                outputs={'user_response': summary} if summary else {},
            )
            self.awaiting_user_response = False
            self.awaiting_user_response_id = None
            self._complete = True
        ...
        expects_reply = any(
            isinstance(action, MessageAction) and action.wait_for_response
            for action in actions
        )
        self.awaiting_user_response = expects_reply
        if expects_reply:
            self.awaiting_user_response_id = (
